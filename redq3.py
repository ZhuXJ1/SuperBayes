...
outdir = '/fred/oz008/xzhu/projects/SuperBayes/PG1302_2020/red_qpo'
...
priors['logCC'] = bilby.core.prior.Uniform(-7, 0, name='logCC', latex_label='$\\ln \\hat{\\sigma}^2$ [mag$^2$ yr$^{-1}$]')
priors['logTAU0'] = bilby.core.prior.Uniform(-4, 4, name='logTAU0', latex_label='$\\ln \\tau_{0}$ [yr]')
priors['Tq'] = bilby.core.prior.Uniform(0.02, 20, name='Tq', latex_label='$T_{q}$ [yr]')
...
class MultidimGaussianLikelihood(bilby.Likelihood):
    ...
    def log_likelihood(self):
        ...
        DS = np.asarray(data['Mag'])[np.newaxis] - np.asarray(self._offset_array)[np.newaxis] # - np.asarray(self._signal_model())[np.newaxis]
...
#        return np.diag(self.data['Magerr']**2)
    ...
    def _red_noise(self):
        return 0.5*np.exp(self.parameters['logCC'])**2*np.exp(self.parameters['logTAU0'])*np.exp(-self.tauij/365.25/np.exp(self.parameters['logTAU0']))*np.cos(2*np.pi*self.tauij/(self.parameters['Tq']*365.25))
